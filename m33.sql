...
-----------------------Student Table--------------------------------------
   ...
   --insert into Student values('jidaa','zein','sdfsdf@@@',1.7,'Engineering','hamdy@gmail.com','MET',1,4,200,200,18);
 ---------------------------Student_Phone----------------------------------
...
insert into Request(type,comment,credit_hours,student_id,advisor_id) values('credit','hi',4,1,18);
--------------------------------------------------------------
-------------------------------MakeUp_Exam---------------------------------
...
CREATE PROC [Procedures_AdvisorRegistration]
    ...
    Select @Advisor_id =  advisor_id from Advisor 
	 ...
	 DECLARE @OutputAdvisorId int;

-- Execute the stored procedure
EXEC Procedures_AdvisorRegistration
    @advisor_name = 'YourAdvisorName',
    @password = 'YourPassword',
    @email = 'YourEmail',
    @office = 'YourOffice',
    @Advisor_id = @OutputAdvisorId OUTPUT;

-- Print or use the output value as needed
PRINT 'Newly created Advisor ID: ' + CAST(@OutputAdvisorId AS varchar);
select * from Advisor
--/////////////////////////////////////////////////////////////////////////////////
...
INSERT INTO dbo.DebugLog (LogMessage, LogTimestamp) VALUES ('Condition met: Inserting into Graduation_Plan', GETDATE());
...
@major varchar(40)
As
INSERT INTO dbo.DebugLog (LogMessage, LogTimestamp) VALUES ('now we are doing great', GETDATE());
...
Go
--exec Procedures_AdvisorViewAssignedStudents @AdvisorID =18, @major='MET';
-------------------------------------------------------------------------------------
